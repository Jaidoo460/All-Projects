...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS && cubic <= MAX_CUBIC_INCHES && lengthInches < MAX_DIMENSION_INCHES && widthInches < MAX_DIMENSION_INCHES && heightInches < MAX_DIMENSION_INCHES && isSurchargePaid == false ){
            return true;
        }else if (weightPounds <= MAX_WEIGHT_POUNDS && cubic <= MAX_CUBIC_INCHES && lengthInches > MAX_DIMENSION_INCHES | widthInches > MAX_DIMENSION_INCHES | heightInches > MAX_DIMENSION_INCHES && isSurchargePaid == true ){
            return true;
        }
        return false;
    }
